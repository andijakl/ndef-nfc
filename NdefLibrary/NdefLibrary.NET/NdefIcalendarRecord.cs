...
namespace NdefLibrary.Ndef
{
    ...
    public class NdefIcalendarRecord : NdefIcalendarRecordBase
    {
        ...
        private NdefAppointment _appointmentData;

        public NdefAppointment AppointmentData
        {
        ...
        public NdefIcalendarRecord(NdefAppointment ap)
        {
            AppointmentData = ap;
        }
        ...
        private Calendar ConvertAppointmentToIcalendar(NdefAppointment ap)
        {
        ...
        {
            var iCalendarText = Encoding.UTF8.GetString(iCalendarPayload);
            var calendarCollection = CalendarCollection.Load(iCalendarText);

            if (calendarCollection.Count >= 1)
            {
                if (calendarCollection.Count >= 2)
                {
                    System.Diagnostics.Debug.WriteLine("Note: the library only parses the first calendar object.");
                }
                ...
                if (curCalendar.Events != null && curCalendar.Events.Count >= 1)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("Note: the library only parses the first event object.");
                    }
                    var evt = curCalendar.Events.First();

                    // Internalize event data
                    var ap = new NdefAppointment
                    {
                        // Event description and main properties
                        Subject = evt.Summary,
                    ...
                    {
                        ap.Organizer = new NdefOrganizer
                        {
                ...
                {
                    throw new NdefException(NdefExceptionMessages.ExIcalendarNoEventFound);
                }
            }
            else
            {
                throw new NdefException(NdefExceptionMessages.ExIcalendarNoCalendarFound);
            }
        }
    }
}
